...
$admin = $row["admin"];
$sql = "SELECT id_emp, nom, prenom, email, sexe, etat_civil, date_embauche FROM employe WHERE 1=1";
$result = $con->query($sql);
?>
<!DOCTYPE html>
<html lang="en">
...
<?php
    ...
    <tbody>";
        if(mysqli_num_rows($result) == 0)
        {
          echo "
            <tr>
              <td colspan='7' style='text-align: center;'>Aucune donnée</td>
            </tr>";
        }
        else
        {
          while($employe = $result->fetch_assoc())
          {
            ...
            <td><button class='btn btn-outline-dark' onclick=\"window.location.href ='http://localhost/pfa/modifyinfos.php?idm=$idm'\">Modifier</button></td>
            </tr>";
          }
          echo "
          </tbody>
